...
class Player {
  ...
  strike (player, enemy, attackDmg) {
    ...
    let demageAmount = Math.ceil(Math.random() * 10);

    // Subtract the enemy health with the damageAmount
    enemy.health -=  demageAmount
    //  Update the game and DOM with updateGame()
    updateGame(p1, p2,game.isOver)
    //  Return a message of 'player name attacks enemy name for damageAmount'
    return `${player.name} attacks ${enemy.name} for ${demageAmount} demage`
  }
  ...
  heal (player) {
    ...
    return `${player.name} hels for ${hpAmount}HP`
  }
}
...
let player1 = new Player('luan', 100, 10);
